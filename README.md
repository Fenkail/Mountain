...
## 文件的读写
### stream
流是一种数据序列，通过流的方式进行程序和数据的交互，会开辟内存缓冲区
### filesystem
### format
## 运算符重载
### Deep Learning & c++
- 生产效率，面相应用，python
- 运行效率，面向系统开发，cpp，底层优化内嵌汇编语言
- 向量vector：direction & magnitude
- 张量：一阶为向量，二阶为矩阵....
- 后续对着pytorch源码分析吧
## 模板初步
### 元编程和泛型编程
...
3. 泛型编程：处理多种数据+程序
### 模板基础
同时处理整数、浮点数、字符。c语言需要三个函数，c++可以进行重载。板设计输入值和输出值相同数据类型的方法。
### 标准模板库（STL）
包含：容器、迭代器、算法、函数对象、空间分配器
- 容器：个体称为元素，可以为各种类型。顺序容器、关联容器、容器适配器
    1. 顺序容器：线性数据结构、多个元素有序集合，有头有尾（1、3、5、7、9） vector、list、deque
    2. 关联容器：快速定位元素的非线性结构 set、map、key/value
    3. 容器适配器：stack、queue